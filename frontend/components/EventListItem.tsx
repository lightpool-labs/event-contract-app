    ...
    <article className="rounded-xl border border-sky-100 bg-white p-5 shadow-sm transition hover:border-sky-300 hover:shadow-md">
      <div className="flex items-start justify-between gap-4">
        <Link href={`/events/${event.slug}`} className="min-w-0 flex-1">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <h2 className="text-lg font-medium text-slate-900">{event.question}</h2>
              ...
              <p className="mt-2 text-xs text-slate-500">
                Deadline: {new Date(event.resolution_deadline * 1000).toLocaleString()}
              </p>
            </div>
          </div>
        </Link>
        ...
        <div className="grid min-w-[280px] shrink-0 grid-cols-2 gap-2 self-end">
          <Link
            href={`/events/${event.slug}/mint-burn?tab=mint`}
          ...
          <Link
            href={`/events/${event.slug}/mint-burn?tab=burn`}
